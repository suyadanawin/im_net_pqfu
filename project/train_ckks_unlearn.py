...
def main():
    ...
    unlearn_root = config["paths"]["unlearn_root"]
    ckpt_dir = os.path.join(unlearn_root, "checkpoints")
    metrics_dir = os.path.join(unlearn_root, "metrics")
    stats_dir = os.path.join(unlearn_root, "stats")
    for p in [unlearn_root, ckpt_dir, metrics_dir, stats_dir]:
        ensure_dir(p)
    ...
    save_json(
        {
            ...
            "base_ckks_checkpoint": config["paths"]["base_ckks_checkpoint"],
        },
        os.path.join(stats_dir, "unlearning_setup.json"),
    )
    ...
    state_dict = load_flexible_state_dict(config["paths"]["base_ckks_checkpoint"], device)
    ...
    metrics_csv = os.path.join(metrics_dir, "ckks_unlearn_round_metrics.csv")
    final_ckpt = os.path.join(ckpt_dir, "ckks_unlearn_final.pt")
    best_ckpt = os.path.join(ckpt_dir, "ckks_unlearn_best.pt")
    ...
    with open(metrics_csv, "w", newline="", encoding="utf-8") as f_csv:
        ...
        for round_idx in range(1, config["unlearning"]["rounds"] + 1):
            print(f"\n===== CKKS Unlearning Round {round_idx}/{config['unlearning']['rounds']} =====")
            ...
            for client_id, loader in enumerate(tqdm(client_loaders, desc="Retained clients")):
                ...
                local_state, metrics = train_one_client(
                    ...
                    dataloader=loader,
                    device=device,
                    epochs=config["unlearning"]["local_epochs"],
            ...
            save_model_state(global_model, final_ckpt)
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                save_model_state(global_model, best_ckpt)

            print(f"Unlearn Val Acc: {val_acc:.2f}%")
    ...
    print("\nCKKS warm-start unlearning finished.")
